// Pure prompt-builder for the optimizer. Extracted from app/api/analyze/route.ts
// so the eval harness (scripts/optimizer-eval.ts) can exercise the SAME prompt
// the production route uses — no drift between what we test and what we ship.
//
// Behavior must remain identical to the in-route version it replaced. If you
// change the scoring rubric, the OPTIMIZED SCORE CONSTRAINTS, or any FAILURE
// CONDITION here, run `npm run eval:optimizer` to confirm the bands still
// hold before merging.

export type DeepDiveAnswers = {
  achievements: string;
  hiddenSkills: string;
  uniqueValue: string;
};
...
export type OptimizerPromptOutput = {
...
export const OPTIMIZER_SYSTEM_PROMPT =
  "You are an executive resume writer who ENHANCES content, not deletes it. Create an AMAZING 3-4 line professional summary. PRESERVE all education details (GPA, honors, coursework). Recent roles: 2 bullets (3 if exceptional), older: 0-1. Transform weak content into strong content. Executive language. NEVER fabricate. Respond with valid JSON only.";
...
export function inferJobTitleFromDescription(desc: string, companyName: string) {
  const lines = desc
    .replace(/\r\n/g, "\n")
    .split("\n")
    .map((l) => l.trim())
...
export function buildOptimizerPrompt(input: OptimizerPromptInput): OptimizerPromptOutput {
  const {
    cvText,
    jobTitle = "",
    jobDescription = "",
