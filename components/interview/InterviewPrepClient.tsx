  ...
  async function unlock() {
    ...
    try {
      ...
      if (res.status === 401 || data?.code === "AUTH_REQUIRED") {
        ...
        toast.message("Create a free account to unlock full prep.");
        openSignUp?.();
        return;
      }
      if (res.status === 402 || data?.code === "INSUFFICIENT_CREDITS") {
        ...
        useFlashSaleStore.getState().recordAction();
        toast.message("You're out of credits", { description: "Top up or grab the Pro offer to unlock." });
        return;
      }
      if (!res.ok) {
  ...
  return (
    <div>
      <header className="mb-6">
        <p className="text-[12px] uppercase tracking-[0.18em] text-[#B8860B] font-semibold">Interview prep</p>
        <h1 className="mt-2 text-3xl font-bold text-[#0A2647]">Walk in ready</h1>
        <p className="mt-2 text-stone-600 max-w-xl">
          Get the questions you&apos;re most likely to face — then unlock STAR-format answers drawn from your own
          experience and a tight personal pitch.
        </p>
      </header>

      {!hasCv ? (
        <div className="rounded-2xl bg-white border border-stone-200 p-6 text-center">
          <p className="text-stone-600">Build your CV first so we can tailor questions to your experience.</p>
          <Link
            href="/build/chat"
        ...
        <>
          <div className="rounded-2xl bg-white border border-stone-200 p-4 flex flex-col sm:flex-row gap-3 sm:items-end">
            <div className="flex-1">
              <label className="block text-[11px] font-medium uppercase tracking-wide text-stone-500 mb-1">Target role</label>
              <input
                value={role}
            ...
            >
              ...
              {questions.length ? "Regenerate" : "Get my questions"}
            </button>
          </div>
            ...
            <div className="mt-6 space-y-3">
              ...
              {!locked && pitch ? (
                <div className="rounded-2xl bg-[#0A2647]/[0.04] border border-[#0A2647]/15 p-4">
                  <div className="text-[11px] uppercase tracking-[0.14em] text-[#0A2647]/60 mb-1">Your 30-second pitch</div>
              ...
              {locked ? (
                <div className="rounded-2xl bg-[#0A2647] text-white p-5 text-center">
                  ...
                  <h2 className="mt-2 text-lg font-bold">Unlock STAR answers + your pitch</h2>
                  <p className="mt-1 text-white/75 text-sm max-w-md mx-auto">
                    Get model answers for every likely question — built from your real experience — plus a 30-second
                    personal pitch.
                  </p>
                  <button
                    type="button"
