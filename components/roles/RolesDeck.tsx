...
export function RolesDeck({
  ...
  async function regenerate() {
    ...
    try {
      ...
      if (!meRes.ok) throw new Error(t("Sign in first"));
      ...
      if (!cvText) {
        toast.error(t("Run an optimization first so we have your CV to work with."));
        return;
      }
      ...
      toast.success(t("{count} CVs generated", { count: data.cards.length }));
    } catch (e) {
  ...
  function unlockCard(card: Card) {
    ...
    startTransition(async () => {
      try {
        ...
        if (!res.ok) throw new Error(t("Unlock failed"));
        setCards((prev) =>
          prev.map((c) => (c.id === card.id ? { ...c, unlocked: true } : c))
        );
        toast.success(t("{role} unlocked", { role: card.role }));
      } catch {
        toast.error(t("Couldn't unlock — try again"));
      }
    });
  }
    ...
    <div className="mt-8 space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="text-xs text-white/55">
          {cards.length === 1
            ? t("{count} card", { count: cards.length })
            : t("{count} cards", { count: cards.length })}{" "}
          ·{" "}
          {credits === 1
            ? t("{credits} credit available", { credits })
            : t("{credits} credits available", { credits })}
        </div>
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {cards.length > 0 ? t("Regenerate") : t("Generate CV deck")}
          </button>
        </div>
      </div>
...
function CardPreview({ content, error }: { content: unknown; error: string | null }) {
  const { t } = useT();
  if (error) {
    return <div className="text-xs text-[#f5b8c8]">{t("Couldn't generate — {error}", { error })}</div>;
  }
  ...
  return (
    <div className="space-y-3">
      ...
      {summary ? (
        ...
        <p className="text-sm text-white/55 italic">{t("No summary generated.")}</p>
      )}
      {skills.length > 0 ? (
